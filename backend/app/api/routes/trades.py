...
async def export_fills(
    ...
    lines = [
        ...
        "",
        "var string[] tradeData = array.new<string>()",
        "",
        "if barstate.isfirst",
    ...
    lines.extend([
        ...
        "",
        "// 使用更高效的方法：只在最后一根K线处理所有交易，避免重复处理",
        ...
        "                    offset = math.max((target_high - target_low) * 0.1, syminfo.mintick * 10)",
        "                    label_y = target_low - offset",
        "                    // 使用匹配到的K线时间，确保标签显示在正确的K线上",
        "                    label.new(matched_bar_time, label_y, buy_text, xloc=xloc.bar_time, style=label.style_label_up, color=color.new(color.green, 0), textcolor=color.white, tooltip=comment)",
        "                else if side < 0 and show_sell_labels",
        '                    sell_text = str.tostring(qty, "#") + "\\n" + str.tostring(price, "#.##")',
        "                    offset = math.max((target_high - target_low) * 0.1, syminfo.mintick * 10)",
        "                    label_y = target_high + offset",
        "                    // 使用匹配到的K线时间，确保标签显示在正确的K线上",
        "                    label.new(matched_bar_time, label_y, sell_text, xloc=xloc.bar_time, style=label.style_label_down, color=color.new(color.red, 0), textcolor=color.white, tooltip=comment)",
        "",
        "plot(na)",
    ])
